...
public class WelcomeServlet extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest _request, HttpServletResponse _response)
        ...
        thisLogLine++;
        System.out.println(thisName + " " + thisLogLine + " Формируем список категорий для боковой панели");
        int category_id;
        if (_request.getParameterMap().isEmpty()) {
            category_id = 0;
        } else {
            category_id = Integer.parseInt(_request.getParameter(HTMLLinks.PARAMETER_CATEGORY_ID));
        }
